...
//
{
	...
	for (member = 0; member < POPSIZE; member++)
	{
		...
		for (int i = 0; i < marketData.size(); ++i) {
			price = callPriceFFT(12, marketData[i].S, marketData[i].K, marketData[i].T, marketData[i].r, x[1], x[2], (x[3] + x[4]*x[4])/ (2 * x[2]), x[4], x[5]);
			err += (marketData[i].price - price)*(marketData[i].price - price);// / fabs(marketData[i].ask - marketData[i].bid);
		}
		err = -err;

		population[member].fitness = err;
	}
	return;
}
//****************************************************************************80
